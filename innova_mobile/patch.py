...
# 1. Update _abrirModalVacacion
new_abrir = '''void _abrirModalVacacion(BuildContext context, {Empleado? empleado, VacacionEmpleado? vacacionToEdit}) {
...
idx_abrir = content.find('void _abrirModalVacacion(BuildContext context')
if idx_abrir != -1:
    content = re.sub(
        r'void _abrirModalVacacion\(BuildContext context,\s*\{int\?\s*empleadoId,\s*VacacionEmpleado\?\s*vacacionToEdit\}\)\s*\{[\s\S]*?\}\s*\}',
        new_abrir,
        content
    )

# 2. Re-write _VacacionBottomSheet 
...
class _VacacionBottomSheetState extends ConsumerState<_VacacionBottomSheet> {
  ...
  void _calcularPeriodosDisponibles() {
    ...
    String inicioStr = widget.empleado.fechaIngreso?.split('T')[0] ?? '';
...
idx_sheet = content.find('class _VacacionBottomSheet extends ConsumerStatefulWidget')
if idx_sheet != -1:
    end_sheet = content.find('class _FaltaBottomSheet', idx_sheet)
    content = content[:idx_sheet] + sheet_code + '\n\n' + content[end_sheet:]

# 3. Replace calls to _abrirModalVacacion
content = content.replace('_abrirModalVacacion(context, empleadoId: empleado.id)', '_abrirModalVacacion(context, empleado: empleado)')
content = content.replace('_abrirModalVacacion(context, empleadoId: widget.empleado.id)', '_abrirModalVacacion(context, empleado: _empleado)')
content = content.replace('_abrirModalVacacion(context, empleadoId: _empleado.id)', '_abrirModalVacacion(context, empleado: _empleado)')
# Fix the edit call
content = re.sub(
    r'_abrirModalVacacion\(context,\s*empleadoId:\s*empleado\.id,\s*vacacionToEdit:\s*vacacion\)',
    '_abrirModalVacacion(context, empleado: empleado, vacacionToEdit: vacacion)',
    content
)

# And another one
content = re.sub(
    r'_abrirModalVacacion\(context,\s*empleadoId:\s*_empleado\.id,\s*vacacionToEdit:\s*vacacion\)',
    '_abrirModalVacacion(context, empleado: _empleado, vacacionToEdit: vacacion)',
    content
)
...
print('Successfully patched empleado_detail_screen.dart')
