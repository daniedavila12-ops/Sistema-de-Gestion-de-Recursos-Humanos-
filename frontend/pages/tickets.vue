<template>
  <div class="min-h-screen flex font-sans" style="background: linear-gradient(135deg, #f8fafc 0%, #f1f5f9 50%, #e2e8f0 100%)">
    ...
    <!-- CONTENIDO PRINCIPAL -->
    <main :class="['w-full overflow-x-hidden transition-all duration-300 flex-1 p-8 overflow-y-auto', isSidebarCollapsed ? 'md:ml-20' : 'md:ml-64']">
      <BreadcrumbNav :crumbs="[{ text: 'Módulo de Tickets' }]">
        <template #right>
          <div class="relative w-full md:w-auto flex justify-end">
            ...
            <!-- Dropdown Menu -->
            <div v-if="dropdownPerfilAbierto" class="absolute right-0 mt-14 w-64 bg-white rounded-2xl shadow-xl border border-slate-100 overflow-hidden z-50 animate-in slide-in-from-top-2 duration-200">
              ...
              <div v-for="(idObj, index) in identidades" :key="index" class="mb-4 relative">
                ...
                <div class="flex items-center gap-2 relative">
                  <div class="relative flex-1">
                    <input v-model="idObj.value" @input="e => onIdentidadInput(e, index)" @focus="onIdentidadFocus(index)" @blur="onIdentidadBlur" type="text" placeholder="Buscar por identidad o nombre..." required
