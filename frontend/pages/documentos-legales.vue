<template>
  <div class="min-h-screen bg-gray-100 flex font-sans overflow-x-hidden">
    ...
    <main :class="['w-full overflow-x-hidden transition-all duration-300 flex-1 p-4 md:p-8', isSidebarCollapsed ? 'md:ml-20' : 'md:ml-64']">
      ...
      <header class="mb-8 flex flex-col md:flex-row justify-between items-start md:items-center bg-white p-5 rounded-3xl shadow-sm border border-slate-100 gap-4">
        <div class="flex items-center gap-4">
          ...
          <div>
            <h1 class="text-2xl md:text-3xl font-black text-slate-800 tracking-tight uppercase">Archivero <span class="text-amber-500">Legal</span></h1>
            <p class="text-slate-500 mt-1 font-medium italic text-sm md:text-base">Repositorio seguro de documentos legales y normativas.</p>
          </div>
        </div>
        <div class="flex gap-3 w-full md:w-auto">
          <button @click="abrirModal" class="w-full md:w-auto justify-center bg-amber-600 hover:bg-amber-500 text-white px-6 py-3 rounded-xl font-black uppercase text-xs transition-all shadow-lg shadow-amber-900/20 flex items-center gap-2">
        ...
        <div class="p-6">
          <div class="flex flex-col sm:flex-row gap-2 mb-6">
            ...
            <button @click="agregarCategoria" :disabled="!nuevaCategoria.trim()"
              class="bg-slate-900 text-white px-4 py-2 sm:py-0 rounded-xl text-sm font-bold hover:bg-slate-800 transition disabled:opacity-50">
              Añadir
            </button>
          </div>
