  ...
  <div class="min-h-screen bg-[#f1f5f9] flex font-sans">
    ...
    <main :class="['w-full overflow-x-hidden transition-all duration-300 flex-1 p-6 overflow-y-auto', isSidebarCollapsed ? 'md:ml-20' : 'md:ml-64']">
      ...
      <div class="flex flex-col lg:flex-row gap-6 items-start">
        <!-- Sidebar del Módulo -->
        <ReportesSidebar :counts="sidebarCounts" :activeFilter="filtroActivo" :reportesList="unfilteredReportesForCounts" @filter-changed="aplicarFiltro" @categorias-actualizadas="actualizarCategorias" class="shrink-0 w-full lg:w-64 rounded-2xl shadow-sm border border-slate-200/80 overflow-hidden" style="height: auto;" />
        ...
        <div class="flex-1 flex flex-col gap-6">
          ...
          <div class="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden">
            ...
            <div class="px-5 pt-4 pb-0 border-b border-slate-100">
              ...
              <!-- Search & Filters Bar -->
              <div class="flex flex-col md:flex-row gap-3 items-center pb-4">
                <!-- Search -->
                ...
                <div class="flex flex-col sm:flex-row items-center gap-2 w-full md:w-auto">
                  ...
                  <div class="relative w-full sm:w-auto">
                    <select v-model="priorityFilter" class="w-full sm:w-auto appearance-none bg-slate-50 border border-slate-200 rounded-xl pl-3 pr-7 py-2 outline-none focus:border-red-300 text-sm font-medium text-slate-600 transition-all cursor-pointer">
                  ...
                  <!-- Sort -->
                  <div class="relative w-full sm:w-auto">
                    <select v-model="sortOption" class="w-full sm:w-auto appearance-none bg-slate-50 border border-slate-200 rounded-xl pl-3 pr-7 py-2 outline-none focus:border-red-300 text-sm font-medium text-slate-600 transition-all cursor-pointer">
              ...
              <div v-for="reporte in reportesPaginados" :key="reporte.id"
                ...
                <div class="flex items-start gap-4">
                  ...
                  <div class="flex-1 min-w-0">
                    ...
                    <h3 class="text-slate-800 font-semibold text-[14px] leading-snug group-hover:text-red-600 transition-colors truncate mb-1 whitespace-normal">
                      {{ reporte.tema || 'Sin tema' }}
                    </h3>
                    <div class="flex flex-col sm:flex-row sm:items-center gap-2 mb-1 mt-1">
        ...
        <div class="p-6">
          <form @submit.prevent="crearReporte" class="space-y-5">
            <div>
              ...
              <div v-for="(id, index) in nuevoReporte.identidades" :key="index" class="flex gap-2 mb-2 relative">
                <div class="relative flex-1">
                  <input :value="nuevoReporte.identidades[index]" type="text" placeholder="Buscar por identidad o nombre..." required
                    @input="e => onIdentidadInput(e, index)"
                    @focus="onIdentidadFocus(index)"
                    @blur="onIdentidadBlur"
...
const selectEmpleado = (emp, index) => {
  nuevoReporte.value.identidades[index] = emp.identidad
